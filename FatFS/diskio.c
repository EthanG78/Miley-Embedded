...
static void deselect(void) {
  ...
  // spi_cs_low();
  spi_exchangeByte(
      0xFF); /* Dummy clock (force DO hi-z for multiple slave SPI) */
}
...
static BYTE rcvr_datablock(BYTE *buff, /* Data buffer to store received data */
  ...
  do *buff++ = spi_exchangeByte(0xFF); /* Receive the data block into buffer */
  while (--btr);
...
static BYTE send_cmd(          /* Returns R1 resp (bit7==1:Send failed) */
  ...
  if (cmd == CMD12)
    spi_exchangeByte(0xFF); /* Skip a stuff byte on stop to read */
...
DRESULT disk_ioctl(BYTE pdrv, /* Physical drive nmuber (0..) */
  ...
  switch (cmd) {
    case CTRL_SYNC: /* Make sure that no pending write process. Do not remove
                       this or written sector might not left updated. */
      ...
      if (CardType & CT_SD2) {          /* SDv2? */
        if (send_cmd(ACMD13, 0) == 0) { /* Read SD status */
          ...
          if (rcvr_datablock(csd, 16)) { /* Read partial block */
            for (n = 64 - 16; n; n--)
              spi_exchangeByte(0xFF); /* Purge trailing data */
